    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImageList="imgList" />
          <!-- 小图列表 -->
          <ImageList :skuImageList="imgList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd @click="changeActive(item,spuSaleAttr.spuSaleAttrValueList)" changepirce="0" :class="{active:item.isChecked==1}" v-for="item in spuSaleAttr.spuSaleAttrValueList" :key="item.id">{{item.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      imgList(){
        // 传递给子组件的zoom的东西，防止报错，至少给一个空数组
        // 就是不能从undefined中读取数据，起码是从空数组，空对象中读取数据
        return this.skuInfo.skuImageList||[]
      }
    },
    methods: {
      // 高亮点击，排他思想,传递点击的item，以及包含当前item的数组（spuSaleAttr.spuSaleAttrValueList）
